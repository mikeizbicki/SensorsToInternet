...
Some resistors(resistance can vary, we used 330 ohm) 
...
The thermistor, on the other hand, is a little more complicated. A thermistor is a temperature dependent resistor, meaning that it has a specific resistance at room temperature. The resistance varies in proportion with temperature of the component. In our case, our thermistor is a negative temperature coefficient thermistor, which is a resistor with its resistance decreasing as the temperature of the surrounding area increases. By including another external resistor along with the thermistor, we created a resistive divider. The resistive divider allows us to calculate the amount of current that flows through both resistors in series. Given the voltage coming from the electric imp, the external 330 ohm resistor we added, and the current that is flowing through both of them in series, we can now calculate the resistance of the thermistor. From the resistance of the thermistor, we can determine the temperature of the surrounding area that the thermistor is reading. The thermistor is connected to pins 7 and 9 of the electric imp. Pin 9 connected to the thermistor is used as an analog to digital converter. It provides a voltage to the thermistor, thus allowing for it to intake analog input from the surrounding environment. Pin 7 is connected on the other side of the thermistor and acts as digital output. When pin7 is set on high, no current flows through the resistors and therefore there is no reading taken in by the sensor. When pin7 is set low, current flows through thus allowing the thermistor to actually take a measurement. Through implementing pin7, we have control when to take a temperature reading from the thermistor. 
...
Photoresistor:
Select a pin of your choice, in our case we have selected pin 8. Connect a resistor of your choice, we used a 330K ohm resistor, and connect that to one end of the photoresistor. Connect the other end of the 330 ohm resistor to power bus on your board. Now connect the other side of the photoresistor that is not in series with the resistor to the ground bus. Depending on the pin you have selected to use, you must write your code accordingly to use this pin or else it will not work. 
...
Outputting Data onto 2 Graphs:

In order to output data onto 2 separate graphs, in the agent section of our code, we wrote to have 2 ```device.on``` functions. One for light readings, and one for temperature readings. By changing the filename to different file names in each function, it will output to 2 separate graphs on plotly. Each time you want to start fresh on a new graph, you can just change the filename, and the terminal will output a different url. If you do not change the filename inside ```device.on```, it will continue to update the same graph and plot the data as well. 
